...
const chimeSDKMediaPipelines = new ChimeSDKMediaPipelines({
  region: 'us-east-1',
  endpoint:
    process.env.CHIME_SDK_MEDIA_PIPELINES_ENDPOINT ||
    'https://media-pipelines-chime.us-east-1.amazonaws.com',
});
      ...
      try {
        ...
        if (request.method === 'GET' && requestUrl.pathname === '/') {
          ...
          respond(
            ...
            'application/json',
            JSON.stringify(debugResponse.response, null, 2)
          );
          ...
          if (!requestUrl.query.title || !requestUrl.query.name) {
            respond(
              ...
              400,
              'application/json',
              JSON.stringify({ error: 'Need parameters: title and name' })
            );
          }
          ...
          if (requestUrl.query.primaryExternalMeetingId) {
            ...
            if (primaryMeeting) {
              console.info(
                `Retrieved primary meeting ID ${primaryMeeting.Meeting.MeetingId} for external meeting ID ${requestUrl.query.primaryExternalMeetingId}`
              );
            ...
            if (!primaryMeeting) {
              respond(
                ...
                400,
                'application/json',
                JSON.stringify({ error: 'Primary meeting has not been created' })
              );
            }
          }
          ...
          if (!meeting) {
            if (!requestUrl.query.region) {
              respond(
                ...
                'application/json',
                JSON.stringify({
                  error: 'Need region parameter set if meeting has not yet been created',
                })
              );
            }
                ...
                await chimeSDKMeetings.startMeetingTranscription({
                  ...
                  TranscriptionConfiguration: {
                    EngineTranscribeSettings: {
                      ...
                      RoleArn: 'arn:aws:iam::881471315585:role/ChimeTranscriptionRole',
                    },
                  },
                });
          ...
          if (meeting.Meeting.PrimaryExternalMeetingId !== undefined) {
            ...
            joinResponse.JoinInfo.PrimaryExternalMeetingId =
              meeting.Meeting.PrimaryExternalMeetingId;
          }
          ...
          try {
            await chimeSDKMeetings.startMeetingTranscription({
              ...
              TranscriptionConfiguration: {
                EngineTranscribeSettings: {
                  ...
                  Region: 'ap-southeast-2',
                  RoleArn:
                    'arn:aws:iam::881471315585:role/aws-service-role/transcription.chime.amazonaws.com/AWSServiceRoleForAmazonChimeTranscription',
                },
              },
            });
            ...
            respond(
              ...
              200,
              'application/json',
              JSON.stringify({ status: 'transcription_started' })
            );
          } catch (err) {
            console.error('Error starting transcription:', err);
            respond(response, 500, 'application/json', JSON.stringify({ error: err.message }));
          }
          ...
          if (ivsEndpoint) {
            try {
                  ...
                  Sources: [
                    {
                      ChimeSdkMeetingLiveConnectorConfiguration: {
                        Arn: `arn:aws:chime::${callerInfo.Account}:meeting:${
                          meetingTable[requestUrl.query.title].Meeting.MeetingId
                        }`,
                        CompositedVideo: {
              ...
              meetingTable[requestUrl.query.title].LiveConnector =
                liveConnectorPipelineInfo.MediaLiveConnectorPipeline;
          ...
          if (ivsEndpoint) {
            liveConnectorPipelineId =
              meetingTable[requestUrl.query.title].LiveConnector.MediaPipelineId;
          ...
          request.method === 'GET' &&
          (requestUrl.pathname === '/audio_file' || requestUrl.pathname === '/stereo_audio_file')
        ) {
          ...
          request.method === 'POST' &&
          requestUrl.pathname === '/update_attendee_capabilities'
        ) {
          const data = await chimeSDKMeetings.updateAttendeeCapabilities({
            MeetingId: meetingTable[requestUrl.query.title].Meeting.MeetingId,
            AttendeeId: requestUrl.query.attendeeId,
            Capabilities: {
          ...
          request.method === 'POST' &&
          requestUrl.pathname === '/batch_update_attendee_capabilities_except'
        ) {
          const data = await chimeSDKMeetings.batchUpdateAttendeeCapabilitiesExcept({
            MeetingId: meetingTable[requestUrl.query.title].Meeting.MeetingId,
          ...
          const getAttendeeResponse = await chimeSDKMeetings.getAttendee({
            MeetingId: meetingTable[requestUrl.query.title].Meeting.MeetingId,
